        ...
        <table id="example" class="display nowrap" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a class="edit" href="#" data-id='{{$item->id}}' data-status='{{$item->status}}' data-rejected_reason='{{$item->rejected_reason}}'>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('admin.leave_approved')}}" method="post" id="password" enctype="multipart/form-data">
                    @csrf
                     <input type="hidden" id="id" name="id">
                     <div>
                            <input type="radio"  name="status" value="1" id=" " required/>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.edit', function(event) {
        ...
        if (status !== undefined && status !== '') {
            if (status == 0) {
                ...
                $("#leave_approved").prop("checked", false);
            } else if (status == 1) {
                $("#leave_approved").prop("checked", true);
                $("#leave_rejected").prop("checked", false);
            }
        } else {
            // If 'status' is not defined or empty, uncheck both radio buttons
            $("#leave_approved").prop("checked", false);
            $("#leave_rejected").prop("checked", false);
        }
    });
    ...
    $('#deletemember').click(function() {
        var id = $(this).val();
        $.post("{{ URL::to('admin/leave_delete_admin') }}", {
                id: id
            },
            function() {
