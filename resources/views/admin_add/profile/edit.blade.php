...
@section('content')
    ...
    <style>
        ...
        .nav-link.active {
            background-color: linear-gradient(270deg, #b30000 0%, #ff6666 100%) !important
        }
        ...
        <div class="row">
                        ...
                        <div class="info-container">
                            <ul class="list-unstyled mb-6">
                                ...
                                <li class="mb-2">
                                    ...
                                    <span>{{ $data->emp_birthday_date ? \Carbon\Carbon::parse($data->emp_birthday_date)->format('d-m-Y') : 'N/A' }}</span>
                                </li>
                                <li class="mb-2">
            ...
            <div class="col-xl-8 col-lg-7 col-md-7 order-0 order-md-1">
                ...
                <div class="nav-align-top">
                    <ul class="nav nav-pills flex-column flex-md-row flex-wrap mb-6 row-gap-2">
                        ...
                        <li class="nav-item">
                            <a class="nav-link waves-effect waves-light tablink" href="javascript:void(0);" id="defaultOpen"
                                onclick="openPage('Page_Access', this, '#d93333')">
                                <i class="ri-notification-4-line me-1_5"></i>Page Access
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link waves-effect waves-light tablink" href="javascript:void(0);"
                                onclick="openPage('Security', this, '#d93333')">
                    ...
                    <div id="Page_Access" class="tabcontent">
                        <form method="POST" action="{{ route('admin.employee.page.access') }}">
                            @csrf
                            <div class="card mb-6">
                                <!-- Notifications -->
                                <h5 class="card-header border-bottom mb-0">Page Access</h5>
                                <div class="table-responsive">
                                    <table class="table" id="example">
                                        <thead>
                                            <tr>
                                                <th class="text-nowrap">Employee</th>
                                                <th class="text-nowrap text-center">Client Data</th>
                                                <th class="text-nowrap text-center">Project Data</th>
                                                <th class="text-nowrap text-center">Invoice</th>
                                                <th class="text-nowrap text-center">Company Progress</th>
                                                <th class="text-nowrap text-center">Client Income</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @foreach ($getEmployee as $employee)
                                                <tr>
                                                    <td class="text-nowrap text-heading">{{ $employee->emo_name ?? '' }}
                                                    </td>

                                                    <!-- Add Employee ID as part of the checkbox name -->
                                                    ...
                                                    <td>
                                                        <div class="form-check mb-0 d-flex justify-content-center">
                                                            ...
                                                            <input type="hidden" name="client_data[{{ $employee->id }}]"
                                                                value="0">
                                                            <input class="form-check-input" type="checkbox" value="1"
                                                                @if (isset($employee->page_role) && $employee->page_role->client_data == 1) checked @endif
                                                                name="client_data[{{ $employee->id }}]"
                                                                id="defaultCheck1_{{ $employee->id }}">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-check mb-0 d-flex justify-content-center">
                                                            <input type="hidden" name="project_data[{{ $employee->id }}]"
                                                                value="0">
                                                            <input class="form-check-input" type="checkbox" value="1"
                                                                @if (isset($employee->page_role) && $employee->page_role->project_data == 1) checked @endif
                                                                name="project_data[{{ $employee->id }}]"
                                                                id="defaultCheck2_{{ $employee->id }}">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-check mb-0 d-flex justify-content-center">
                                                            <input type="hidden" name="invoice[{{ $employee->id }}]"
                                                                value="0">
                                                            <input class="form-check-input" type="checkbox" value="1"
                                                                @if (isset($employee->page_role) && $employee->page_role->invoice == 1) checked @endif
                                                                name="invoice[{{ $employee->id }}]"
                                                                id="defaultCheck3_{{ $employee->id }}">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-check mb-0 d-flex justify-content-center">
                                                            <input type="hidden"
                                                                name="company_progress[{{ $employee->id }}]"
                                                                value="0">
                                                            <input class="form-check-input" type="checkbox"
                                                                value="1"
                                                                @if (isset($employee->page_role) && $employee->page_role->company_progress == 1) checked @endif
                                                                name="company_progress[{{ $employee->id }}]"
                                                                id="defaultCheck4_{{ $employee->id }}">
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="form-check mb-0 d-flex justify-content-center">
                                                            <input type="hidden"
                                                                name="client_income[{{ $employee->id }}]" value="0">
                                                            <input class="form-check-input" type="checkbox"
                                                                value="1"
                                                                @if (isset($employee->page_role) && $employee->page_role->client_income == 1) checked @endif
                                                                name="client_income[{{ $employee->id }}]"
                                                                id="defaultCheck5_{{ $employee->id }}">
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
                                ...
                                <div class="card-body">
                                    <button type="submit" class="btn me-3 waves-effect waves-light"
                                        style="background: linear-gradient(270deg, #b30000 0%, #ff6666 100%) !important;color: #f0f8ff;">Save
                                        changes</button>
                                </div>
                                <!-- /Notifications -->
                            </div>
                        ...
                        <div class="modal-body p-0">
                            ...
                            <form action="{{ route('admin.profile.update') }}" method="post" id="editUserForm"
                                ...
                                <div class="col-12 col-md-6">
                                    <div class="form-floating form-floating-outline">
                                        <input type="text" readonly class="form-control dob-picker"
                                            name="emp_birthday_date" id="emp_birthday_date"
                                             value="{{ $data->emp_birthday_date ? \Carbon\Carbon::parse($data->emp_birthday_date)->format('d-m-Y') : '' }}"
                                ...
                                <div class="col-12 col-md-12">
                                    <div class="form-floating form-floating-outline">
                                        <textarea name="address" class="form-control" style="height: 102px;" placeholder="Enter Address">{{ $data->emp_address ?? '' }}</textarea>
                                        @error('emp_birthday_date')
                                            <div class="text-danger fw-semibold">{{ $message }}</div>
                                        @enderror
                                        ...
                                        @if (isset($data) && !empty($data->profile_image) && file_exists(public_path('profile_image/' . $data->profile_image)))
                                            ...
                                            <div class="icon-container" style="position: absolute;top:1px;left:130px;">
                                                <a href="{{ asset('profile_image/' . $data->profile_image) }}"
                                                    style="background: linear-gradient(270deg, #b30000 0%, #ff6666 100%) !important;color:#fff;border: none;"
        ...
        <script>
            document.querySelector('form').addEventListener('submit', function() {
                ...
                document.querySelectorAll('input[type="checkbox"]').forEach(function(checkbox) {
                    // If a checkbox is not checked, create a hidden input with the same name and value of "0"
                    if (!checkbox.checked) {
