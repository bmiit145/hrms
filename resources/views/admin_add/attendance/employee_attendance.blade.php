...
@section('content')
    <style>
        ...
        .dataTables_wrapper .dataTables_paginate .paginate_button.current .attendance-cell ,
        .dataTables_wrapper .dataTables_paginate .paginate_button.current:hover .attendance-cell{
            color: #fff !important;
        }
                    ...
                    <form method="GET" action="{{ route('admin.employee.attendance') }}">
                        <div class="row">
                            <div class="col-md-3">
                                ...
                                <select name="month" id="month" class="form-control employee-id">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="year" id="year" class="form-control employee-id">
...
if ($attendanceForDay) {
    ...
    $bgColor = ''; // No background if no attendance data
                                    ...
                                    <td>
                                        ...
                                        @if ($workingDays > 0)
                                            <strong>{{ number_format($attendances->first()->get_emp_name->monthly_selery / $workingDays) }}
                                            </strong>
                                        @else
                                            <strong>{{ number_format(0) }} </strong>
                                        @endif
                                    ...
                                    <td class="grand-total">
                                        @php
                                            ...
                                            $perday =
                                                is_numeric($monthlySalary) && $monthlySalary > 0
                                    ...
                                    <td><strong>{{ number_format($otAmount, 0) }}</strong></td>
                                    <td><strong>{{ number_format($Benefits, 0) }}</strong></td>
                                    <td>
                                        <span class="export-value"
                                            style="display: none;">{{ $dedunction->amont ?? '' }}</span>
                                        <form action="{{ route('admin.storeAdjustment') }}" method="POST"
                                            style="display: flex;">
                                            @csrf
                                            <input type="hidden" value="{{ $dedunction->id ?? '' }}" name="id">
                                            <input class="form-control" style="height: 30px;" type="text"
                                                value="{{ $dedunction->amont ?? '' }}" name="amont"
                                                placeholder="Enter value">
                                            <input type="hidden" value="{{ $attendances->first()->get_emp_name->id }}"
                                                name="emp_id">
                                            <input type="hidden" name="datea_and_month"
                                                value="{{ request('month', Carbon\Carbon::now()->month) }}">
                                            <input type="hidden" name="date_and_year"
                                                value="{{ request('year', Carbon\Carbon::now()->year) }}">
                                    ...
                                    <td> <strong>{{ $getpl }}</strong></td>
                                    <td>
                                        @php
                                            $subTotal =
                                                $grandTotal +
                                                (is_numeric($otAmount) ? $otAmount : 0) +
                                                (is_numeric($Benefits) ? $Benefits : 0) +
                                            ...
                                            $year = request('year', Carbon\Carbon::now()->year);
                                            $perday =
                                                $attendances->first()->get_emp_name->monthly_selery / $workingDays;
                                        @endphp
                                        @if ($getpl == 0)
                                            <strong>{{ number_format(floatval($subTotal) + floatval($perday)), 0 }}</strong>
                                        @else
                                            <strong>{{ number_format($subTotal, 0) }}</strong>
                                        @endif
                                    </td>
                                    <td>
                                        <a href="{{ url('admin/salary-slip-priview/' . $attendances->first()->get_emp_name->id . '/' . $month . '/' . $year) }}"
                                            target="_blank">
