...
<style>
    ...
    .user-name {
        font-size: 1.5rem;
        font-weight: bold;
        margin: 0 0 10px 0; /* Ensure no extra top margin */
    }
    ...
    .attendance-status {
        font-size: 1.3rem;
        font-weight: bold;
        margin: 0 0 15px 0; /* Ensure no extra top margin */
    }
    ...
    img.user-image.absent {
        border: 3px solid #fc3f41;
    }
    img.user-image.present {
...
<div class="attendance-container">
 ...
 <!-- Attendance Status -->
    @php
        ...
        $attendance = $data->firstWhere('today_date', $today);
    @endphp
    ...
    <img src="{{ (auth()->user()->profile_image && file_exists(public_path('profile_image/' . auth()->user()->profile_image))) 
     ...
     alt="User Image" 
     class="user-image {{ $attendance && $attendance->status == 1 ? 'present' : 'absent' }}">

   
    <div class="attendance-status {{ $attendance && $attendance->status == 1 ? 'present' : 'absent' }}">
        @if ($attendance && $attendance->status == 1)
            Today Are You Present {{ auth()->user()->emo_name ?? 'User' }} 
        @else
            Today Are You Absent {{ auth()->user()->emo_name ?? 'User' }} 
        @endif
    </div>
    ...
    <!-- Navigation Link -->
    <a href="{{ route('user.user_attendance_sheet') }}" class="attendance-link">
