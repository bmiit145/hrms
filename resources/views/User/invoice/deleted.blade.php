                        ...
                        <tbody>
                            @foreach ($deletedInvoices as $key => $invoice)
                                <tr>
                                    ...
                                    <td>{{ $invoice->total }}</td>
                                    <td>
                                        <form action="{{ route('user.invoice.restore', $invoice->id) }}" method="POST">
                                            @csrf
                                            @method('POST')
                                            <button type="submit" class="btn btn-sm" style="background: linear-gradient(270deg, #b30000 0%, #ff6666 100%) !important;color: #fff;"><i class="ri-reset-left-line" style="font-size: 15px;"></i></button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
