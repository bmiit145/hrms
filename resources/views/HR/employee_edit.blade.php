...
<div class="row">
    <div class="col-xl">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="{{route('hr.update', $data->id)}}" method="post" id="inquiry_form"
        ...
        $('.myForm').validate({ // initialize the plugin
            ...
            rules: {
                ...
                emp_email: {
                    ...
                    email: true,
                    remote: {
                        url: '{{ route('hr.checkEmail') }}', // URL to the route that checks email
                        type: "POST",
                        data: {
                            _token: '{{ csrf_token() }}',
                            email: function () {
...
<script>
    $(document).ready(function () {
        ...
        $("#confirmDeleteBtn").click(function () {
            $.ajax({
                url: '{{route('hr.delete-emp-document')}}',  // Backend route for deleting the document
                type: 'DELETE',
                data: {
    ...
    $("#confirmDeleteSecondBtn").click(function () {
        $.ajax({
            url: '{{route('hr.delete.emp.bank.document')}}',  // Backend route for deleting the document
            type: 'DELETE',
            data: {
                _token: '{{ csrf_token() }}',  // CSRF token for security
                document: deleteDocument,
    ...
    $("#profileConfirmDelete").click(function () {
        $.ajax({
            url: '{{route('hr.delete.emp.profile.document')}}',  // Backend route for deleting the document
            type: 'DELETE',
            data: {
                _token: '{{ csrf_token() }}',  // CSRF token for security
                document: deleteDocument,
