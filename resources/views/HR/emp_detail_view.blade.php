...
@section('content')
    ...
    <style>
        ...
        body.dark-mode .employee-detail h4 strong {
            color: #b0acc7 !important;
        }

        body.dark-mode .employee-detail h4 {
            color: #b0acc7 !important;
        }
    ...
    <div class="employee-detail">
        <h4>
            <strong>{{ $data->emo_name ?? '' }}'s</strong> Information
        </h4>
        <div class="profile-image">
            ...
            @if ($imageExists && !empty($data->profile_image))
                <div style="position: relative; display: inline-block; width: 100px;">
                    <img class="profile-img" src="{{ asset('profile_image/' . $data->profile_image) }}" width="100"
                        height="100" alt="Profile Image" data-src="{{ asset('profile_image/' . $data->profile_image) }}"
        ...
        <div class="employee-info">
            ...
            <div class="info-item">
                ...
                <p id="employee-department">
                    ...
                    null
                    @else
                    {{ isset($data->joining_date) && $data->joining_date ? \Carbon\Carbon::parse($data->joining_date)->format('m-d-y') : 'No Joining Date available' }}
                    @endif
                </p>
                <hr>
            </div>
            ...
            <div class="info-item">
                ...
                <p id="employee-hire-date">
                    @if ($data->emp_birthday_date == 'null')
                    null
                    @else
                    {{ isset($data->emp_birthday_date) && $data->emp_birthday_date ? \Carbon\Carbon::parse($data->emp_birthday_date)->format('m-d-y') : 'No Birthday Date available' }}
                    @endif
                </p>
                <hr>
            </div>
            ...
            <div class="info-item">
                ...
                <p id="employee-hire-date">{{ $data->emp_father_mobile_no ?? 'No Father Mobile Number available' }}</p>
                <hr>
            </div>

            <div class="info-item">
                <label for="employee-hire-date">Mother Mobile Number</label>
                <p id="employee-hire-date">{{ $data->emp_mother_mobile_no ?? 'No Mother Mobile Number available' }}</p>
                <hr>
            </div>
            ...
            <div class="info-item">
                ...
                <p id="employee-hire-date">
                    {{ $data->emp_brother_sister_mobile_no ?? 'No Brother/Sister Mobile Number available' }}</p>
                <hr>
            </div>

            <div class="info-item">
                <label for="employee-hire-date">Monthly Salary</label>
                <p id="employee-hire-date">{{ $data->monthly_selery ?? 'No Salary information available' }}</p>
                <hr>
            </div>

            <div class="info-item">
                <label for="employee-hire-date">Bank Number</label>
                <p id="employee-hire-date">{{ $data->bank_no ?? 'No Bank Number available' }}</p>
                <hr>
            </div>

            <div class="info-item">
                <label for="employee-hire-date">Bank Name</label>
                <p id="employee-hire-date">{{ $data->bank_name ?? 'No Bank Name available' }}</p>
                <hr>
            </div>
                    ...
                    @if ($imageExists && !empty($data->emp_bank_document))
                        ...
                        <div style="position: relative; display: inline-block; width: 100px;">
                            <img class="profile-img" src="{{ asset('emp_bank_document/' . $data->emp_bank_document) }}"
                                width="100" height="100" alt="Bank Document"
                                data-src="{{ asset('emp_bank_document/' . $data->emp_bank_document) }}"
                                data-bs-toggle="modal" data-bs-target="#imageModal" />
                            <!-- Download Button -->
                            <a href="{{ asset('emp_bank_document/' . $data->emp_bank_document) }}" class=" btn-sm "
                                download title="Download" style="position: absolute; top: 5px; left: 5px;color: #7e4ee6;">
            ...
            <div class="row col-12 col-md-4">
                <div class="info-item">
                    <label for="">Employee Documents</label>
                </div>
                <div class="profile-image">
                            ...
                            @foreach ($empDocuments as $index => $document)
                                ...
                                @if (!empty($document) && file_exists($filePath))
                                    <div class="document-item mb-2"
                                        style="position: relative; display: inline-block; width: 120px; margin: 10px;">
                                        @if (in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg']))
                                            <!-- Image Thumbnail -->
                                            <img class="preview-img" src="{{ asset('emp_document/' . $document) }}"
                                                width="100" height="100" alt="Document" data-bs-toggle="modal"
                                                data-bs-target="#imageModal"
                                                data-src="{{ asset('emp_document/' . $document) }}" />
                                        @elseif($extension == 'pdf')
                                            <!-- PDF Icon -->
                                            <div class="pdf-icon-container" style="cursor: pointer;">
                                                <img src="{{ asset('assets/img/icons/misc/pdf.png') }}" alt="PDF Icon"
                                                    class="preview-pdf" data-bs-toggle="modal" data-bs-target="#pdfModal"
                                        ...
                                        <div class="icon-container" style="position: absolute; top: 5px; right: 5px;">
                                            <a href="{{ asset('emp_document/' . $document) }}" class="btn-sm" download
                                                title="Download">
                                                <i class="fa fa-download"></i>
                                            </a>
                                        </div>
    ...
    <div class="modal fade" id="pdfModal" tabindex="-1" aria-labelledby="pdfModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="height: 1000%; width: 100%">
                ...
                <div class="modal-body">
                    <iframe id="pdfPreview" src="" width="100%" height="700px"
